...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  // 1. Properly await params for Next.js 15 (Fixes the build error)
  const resolvedParams = await params;
  const slug = decodeURIComponent(resolvedParams?.slug || "");

  // 2. Fetch data from the 'link' column (Verified in your Supabase screenshot)
  ...
  return (
    <div className="min-h-screen bg-white text-black p-10 font-sans">
      <Link href="/" className="text-blue-600 hover:underline mb-8 block font-bold transition-colors">
        ← Return to Homepage
      </Link>
        ...
        {courses && courses.length > 0 ? (
          courses.map((course) => (
            <div key={course.id} className="border-4 border-black p-8 bg-gray-50 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
              ...
              {/* THE DIRECT LINK: RESTORED & RESPONSIVE */}
              <a 
                href={course.link || "#"} 
                target="_blank" 
                rel="noopener noreferrer"
                className="inline-block w-full sm:w-auto bg-black text-white px-10 py-4 font-black uppercase tracking-tighter transition-all duration-200 text-xl text-center
                           hover:bg-blue-600 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] 
