...
export default function CourseHub() {
  ...
  useEffect(() => {
    async function getCourses() {
      // Limit set to 400 for your target
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-5 py-2 rounded-xl text-sm font-bold transition-all ${
                  selectedCategory === cat ? "bg-blue-600 text-white shadow-lg" : "bg-white text-gray-600 border border-gray-200 hover:bg-gray-100"
                }`}
              >
                {cat}
              </button>
          ...
          {loading ? (
            <div className="text-center py-20 font-bold text-gray-400">Loading Resources...</div>
          ) : (
              ...
              {filteredCourses.map((course) => (
                <div key={course.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col hover:shadow-xl transition-all">
                  ...
                  <div className="p-6">
                    ...
                    <a href={course.url} target="_blank" className="mt-4 block text-center py-3 bg-gray-900 text-white rounded-xl font-bold hover:bg-blue-600">Start Learning →</a>
                  </div>
                </div>
              ))}
            </div>
